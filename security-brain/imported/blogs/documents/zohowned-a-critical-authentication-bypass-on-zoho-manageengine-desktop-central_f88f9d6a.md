...
original_path: knowledge-inbox/blogs-incoming/2022-01-20_zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central.md
original_filename: 2022-01-20_zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central.md
title: 'ZohOwned :: A Critical Authentication Bypass on Zoho ManageEngine Desktop
  Central'
category: documents
detected_topics:
- command-injection
...
- password-reset
language: en
raw_sha256: f88f9d6a8207c694e0f09e31d17aaf1563c6590cdd54e89e4d14de52a93ec846
text_sha256: de7f5f1f8eab0d414ec620fd01e06a95a50054a8e92d82b1d88af62ffbc9dde6
...
- Original Path: knowledge-inbox/blogs-incoming/2022-01-20_zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central.md
- Source Type: markdown
- Detected Topics: command-injection, api-security, cloud-security, file-upload, path-traversal, password-reset
- Ingested At: 2026-06-28T07:32:09Z
- Redactions Applied: True
- Raw SHA256: `f88f9d6a8207c694e0f09e31d17aaf1563c6590cdd54e89e4d14de52a93ec846`
- Text SHA256: `de7f5f1f8eab0d414ec620fd01e06a95a50054a8e92d82b1d88af62ffbc9dde6`
...
final_url: "https://srcincite.io/blog/2022/01/20/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central.html"
...
![Desktop Central](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/logo.png)
...
At _[1]_ the code calls `stateParserGenerator.processState` with the attacker controlled request:
...
In order to survive `StateParserGenerator.processState`, the attacker will need to populate the `TreeSet` at _[2]_ with a `STATE_COOKIE` at _[3]_ so that they don’t crash and burn at _[4]_. Also, the attacker needs to use `StateParserGenerator.processState` method at _[5]_ to craft a special `state` map containing values to survive _[6]_ and _[7]_. There is no way to return null from `StateParserGenerator.parseState`, I already thought of that!

Once the attacker can proceed past `StateParserGenerator.processState`, they can set `forwardPath` at _[8]_ with the provided URI and subsequently set `path` at _[9]_
...
Now, the code at _[10]_ and _[11]_ of the `StateFilter.doFilter` method forwards the incoming request and bypasses any further filters or interceptors within the filter chain. The fact that the forward happens inside of a filter is very powerful, it means that any HTTP verb can be used to reach dangerous API.
...
At _[1]_ and _[2]_ the code gets the `computerName` and `filename` parameters from the incoming request and then at _[3]_ the code builds a path using the controlled `computerName`. Then at _[4]_ the code calls `FileUploadUtil.hasVulnerabilityInFileName` using `zip|7z|gz` as a filter:
...
The code checks that the file extension is either zip, 7z or gz with a check for a traversal but there is no check for a traversal in the `localDirToStore` at _[5]_ which is later used for a controlled write at _[6]_.
...
![Loading a zip from the lib directory](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/libload.png)

In fact, it can be _any_ extension and it clearly not mentioned in the [Tomcat documentation](https://tomcat.apache.org/tomcat-7.0-doc/class-loader-howto.html)! This inturn loaded a malicious jar file (hidden as a zip file) which overwrote core classes. When those classes were loaded from the server/process upon a restart, then their code would execute.

The threat actors also used the `/fos/statuscheck` endpoint which safety returned the string `OK` if the server was up.

![Checking the status of the server](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/status.png)
...
There are 4 main limitations with the attack chain used by the threat actors:
  ...
  1. The `StateFilter` arbitrary forward is _only_ a partial authentication bypass. It’s possible to reach the servlet endpoints, but not possible to reach any of the REST api or struts ActionForward classes. This is a significate weakness in the attack.
...
I finally managed to find a better way to (ab)use the `StateFilter` arbitrary forward by reaching the `ChangeAmazonPasswordServlet`. At first I ignored this servlet because I thought, what’s the point of changing an Amazon password anyway.
  ...
  /*  */  
  /* 33 */  String newUserPassword=***REDACTED***newUserPassword");
  /*  */  
  /* 35 */  SYMClientUtil.changeDefaultAwsPassword(loginName, newUserPassword); // 1
  

At _[1]_ the code calls `SYMClientUtil.changeDefaultAwsPassword` using the attacker supplied `loginName` and `newUserPassword`:
...
When I saw _[2]_ I got very suspicious because I saw `AuthUtil.changePassword`. When I was auditing previously, I remember seeing that function used for other password reset functionality so I decided to do a quick xref on it:

![Other functions that call changePassword](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/changePassword.png)

_Could this code change the admin password from an unauthenticated context? Yes!_

![Changing the admin password unauthenticated](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/haxed.png)
...
![Accessing agents that are connected to Desktop Central](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/agent-access.png)

![Popping a SYSTEM shell over the web interface](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/agent-shell.png)
...
![Changing the admin password as a guest user on the latest version \(10.1.2138.1\)](/assets/images/zohowned-a-critical-authentication-bypass-on-zoho-manageengine-desktop-central/haxed-as-guest.png)
