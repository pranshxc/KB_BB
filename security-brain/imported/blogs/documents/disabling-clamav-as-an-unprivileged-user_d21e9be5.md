...
original_path: knowledge-inbox/blogs-incoming/2023-02-19_disabling-clamav-as-an-unprivileged-user.md
original_filename: 2023-02-19_disabling-clamav-as-an-unprivileged-user.md
...
- api-security
language: en
raw_sha256: d21e9be5ff30bd7ceff43ce33b1026d5e3ef27a3a0fb0acba89f07a98ba20a79
text_sha256: 5d989b21d711928c95f204f003956581decb52154f26c2bc1e68b113f4ab461d
...
- Original Path: knowledge-inbox/blogs-incoming/2023-02-19_disabling-clamav-as-an-unprivileged-user.md
- Source Type: markdown
- Detected Topics: command-injection, supply-chain, sso, access-control, api-security
- Ingested At: 2026-06-28T07:32:18Z
- Redactions Applied: False
- Raw SHA256: `d21e9be5ff30bd7ceff43ce33b1026d5e3ef27a3a0fb0acba89f07a98ba20a79`
- Text SHA256: `5d989b21d711928c95f204f003956581decb52154f26c2bc1e68b113f4ab461d`
...
---
title: "Disabling ClamAV as an Unprivileged User"
page_title: "Disabling ClamAV as an Unprivileged User · Arch Cloud Labs"
url: "https://www.archcloudlabs.com/projects/disabling-clamav-as-unprivileged-user/"
final_url: "https://www.archcloudlabs.com/projects/disabling-clamav-as-unprivileged-user/"
...
![logo_clamav.png](/projects/disabling-clamav-as-unprivileged-user/logo_clamav.png)
...
![pr.png](/projects/disabling-clamav-as-unprivileged-user/pr.png)

## Creating The Environment

ClamAV is available in the majority of repositories for Linux distributions. Simple installing with your package manager should lead you to setting up an environment described below. For my demonstration, I’ll be using [Vagrant](https://vagrantup.com) to create a clean Arch Linux environment to test with. This is accomplished with the following commands below.
...
According to the pull request, any user can interact with the ClamAV socket to disable the daemon. With `clamd`, the ClamAV daemon running in the background we’ll explore open files this process has to identify the attack surface.
  
  
  $> lsof -p `pidof clamav`
  

The [lsof](https://man7.org/linux/man-pages/man8/lsof.8.html) command lists open files of a given process, user, etc… depending on user specified flags. The command above will list all open files for a given process id. The string captures within the backticks of `pidof clamav` will return a integer indicating the process id of clamav, and then give the open files of the process. The abbreivated image below shows a unix socket, hmmmmmmm let’s investigate further!

![clam_av_disable.png](/projects/disabling-clamav-as-unprivileged-user/clam_av_disable.png)

As a refresher, [sockets](https://man7.org/linux/man-pages/man2/socket.2.html) are a form of communication on Linux. Typically, when talking about sockets, one tends to think about network communication, AF_INET sockets are used for network based communication. However, [Unix sockets](https://man7.org/linux/man-pages/man7/unix.7.html) enable the developer to use the same type of syscalls (read/write/etc…) for achieving interprocess communication without listening on a port and handling all of the additional overhead of sending data over a network connection. Just like poking at open ports with netcat, we can also leverage netcat to interact with unix sockets and start understanding our attack surface with ClamAV.

Next steps, lets see the file permissions on this socket file. ![clamd_file_perms.png](/projects/disabling-clamav-as-unprivileged-user/clamd_file_perms.png)
...
![shutdown_clam.png](/projects/disabling-clamav-as-unprivileged-user/shutoff_clam.png)
...
The Metasploit framework contains an [auxiliary module](https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/scanner/misc/clamav_control.rb) for executing commands against the ClamAV daemon **if** the daemon is listening on an interface (AF_INET socket). However, in this situation we want to execute against a unix socket, with less than 60 lines of Ruby, we have a new post-execution module that is captured below and [in my repo as well](https://github.com/rapid7/metasploit-framework/blob/f61c3bcefc376d72631ae4f8fcfa5895d462d44c/modules/post/linux/manage/disable_clamav.rb).
...
When reading about CVEs in Open Source software, checking open issues and pull requests may lead to more interesting discoveries. For bug hunting/vulnerability research, the journey of researching one given bug may lead to a fun new discovery. If you enjoyed this blog, reading No Starch Press’ [A Bug Hunter’s Diary](https://nostarch.com/bughunter) may also be something you find interesting. Thank you for reading, please consider sharing if you found this blog informative.

## Update

The metasploit module described above was merged into Metasploit’s master branch after some revisions. The full exchange can be seen [here](https://github.com/rapid7/metasploit-framework/pull/17672).

## References

  * [ClamAV_Control](https://github.com/rapid7/metasploit-framework/blob/master/modules/auxiliary/scanner/misc/clamav_control.rb)
  * [ClamAV-PR-347](https://github.com/Cisco-Talos/clamav/pull/347)
  * [Metasploit Contributors guide](https://docs.metasploit.com/docs/development/get-started/contributing-to-metasploit.html)
