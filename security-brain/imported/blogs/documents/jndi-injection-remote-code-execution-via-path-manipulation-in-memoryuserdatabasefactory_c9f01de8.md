...
original_path: knowledge-inbox/blogs-incoming/2024-07-21_jndi-injection-remote-code-execution-via-path-manipulation-in-memoryuserdatabase.md
original_filename: 2024-07-21_jndi-injection-remote-code-execution-via-path-manipulation-in-memoryuserdatabase.md
...
- api-security
language: en
raw_sha256: c9f01de889ec7a3cacd2a03970f3db5e21f138ec54bed8c553544f4de2ac416b
text_sha256: b6e68199cd85f430e3d181cf2213b17e07d0337b5cbf283df4eea4055c55a146
...
- Original Path: knowledge-inbox/blogs-incoming/2024-07-21_jndi-injection-remote-code-execution-via-path-manipulation-in-memoryuserdatabase.md
- Source Type: markdown
- Detected Topics: supply-chain, command-injection, sso, otp, automation-abuse, api-security
- Ingested At: 2026-06-28T07:32:35Z
- Redactions Applied: False
- Raw SHA256: `c9f01de889ec7a3cacd2a03970f3db5e21f138ec54bed8c553544f4de2ac416b`
- Text SHA256: `b6e68199cd85f430e3d181cf2213b17e07d0337b5cbf283df4eea4055c55a146`
...
final_url: "https://srcincite.io/blog/2024/07/21/jndi-injection-rce-via-path-manipulation-in-memoryuserdatabasefactory.html"
...
![](/assets/images/jndi-injection-rce-via-path-manipulation-in-memoryuserdatabasefactory/logo.jpg)
...
Some interesting code stands out here, at _[1]_ we can see that an attacker can control the `pathname` property on the `MemoryUserDatabase` instance.

At _[2]_ an attacker can also disable the `readonly` setting as well. But the interesting code appears at _[3]_ with the call to `open` on the database instance. Let’s check it out:
...
The above XML will add the role “admin” to the `roles` Map inside of the `MemoryUserDatabase` instance. Returning back to `getObjectInstance`, if the attacker disables read-only at _[6]_ then they can reach `save` at _[7]_.
...
At _[8]_ the code calls `isWriteable`:
...
This code will return _true_ if the path supplied exists and is a directory and finally, if it’s writeable. But how would an attacker achieve this if they used a remote URI such as: `http://attacker.tld/tomcat-users.xml`?
...
So, if an attacker has an arbitrary directory creation primitive, then they can pass this check! Once an attacker has passed the check, they are able to reach _[9]_ which creates the controlled filename with the `.new` extension added to the end. At _[10]_ the attacker controlled write occurs and at _[11]_ , the file is renamed to the original name, chomping the `.new` extension.
...
But of course, what is the attacker going to write? It turns out that they can’t use double quotes or angle brackets due to XML node parsing in the `Digester` class. The attacker can, of course, side step that problem by using _expression language_ if they were to write a JSP file.
...
![](/assets/images/jndi-injection-rce-via-path-manipulation-in-memoryuserdatabasefactory/rce.png)
...
This gives a nice alternative to the typical `BeanFactory` \+ `ELProcessor`/`GroovyShell` combo which maybe required when `ELProcessor` or `GroovyShell` are not available but it _does_ require JSP execution in the target context.
