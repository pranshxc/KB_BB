...
original_path: knowledge-inbox/blogs-incoming/2023-06-30_multiple-vulnerabilities-in-cockpit-cms-v252.md
original_filename: 2023-06-30_multiple-vulnerabilities-in-cockpit-cms-v252.md
...
- command-injection
language: en
raw_sha256: 4f8275357af24f2cedb6c2586ebc5f41849df0a6fe0685b9dc7f4bca83b3065d
text_sha256: 8a567980448f7eb37acd483a725f7cd54eb087f02c86fd673596478e1bef5a89
...
- Original Path: knowledge-inbox/blogs-incoming/2023-06-30_multiple-vulnerabilities-in-cockpit-cms-v252.md
- Source Type: markdown
- Detected Topics: idor, api-security, oauth, xss, sqli, command-injection
- Ingested At: 2026-06-28T07:32:24Z
- Redactions Applied: True
- Raw SHA256: `4f8275357af24f2cedb6c2586ebc5f41849df0a6fe0685b9dc7f4bca83b3065d`
- Text SHA256: `8a567980448f7eb37acd483a725f7cd54eb087f02c86fd673596478e1bef5a89`
...
---
title: "Multiple Vulnerabilities In Cockpit CMS <= V2.5.2"
page_title: "Multiple Vulnerabilities in Cockpit CMS <= v2.5.2"
url: "https://www.ghostccamm.com/blog/multi_cockpit_vulns/"
final_url: "https://www.ghostccamm.com/blog/multi_cockpit_vulns/"
...
![](/images/blog/multi_cockpit_vulns/artur-patching-vulns.gif)
...
![](/images/blog/multi_cockpit_vulns/testlink.png)

Then I simply created a new object using the Admin UI and took a look at the request. 

![](/images/blog/multi_cockpit_vulns/testlink-request.png)

_Hmmmmm_

That is very interesting that in the user's JSON request the ` _model ` and ` _id ` values are set. This lead to my next question: 

_Does the server actually validate the type of link that the user sends is the same as the one configured on the model?_
...
![](/images/blog/multi_cockpit_vulns/testlink-test1.png)
...
![](/images/blog/multi_cockpit_vulns/hmmmm.gif)
...
![](/images/blog/multi_cockpit_vulns/posts-collection.png)
...
![](/images/blog/multi_cockpit_vulns/setting-the-payload.png)
...
![](/images/blog/multi_cockpit_vulns/idor-conf1.png)

Now let the magic begin when I whack in ` ?populate=1 ` into the URL to trigger the vulnerable populate function. 

![](/images/blog/multi_cockpit_vulns/idor-conf2.png)

![](/images/blog/multi_cockpit_vulns/bam-peanut-butter-and-jam.gif)

_However, I am not done yet._

The only issue is that in order to exploit this IDOR vulnerability, you need the name of the model to dump and the Object ID of the data to dump. Now the model name could be guessable, but that Object ID is not and _appears_ to be random. 

_Or is it..._

##  MongoDB Object IDs are Not Secure 

The thing about MongoDB Object IDs is that although that they look like they are _'randomly'_ generated, they are not secure and under certain circumstances can be guessable. Well tbh I got conflicting details [ from this stack overflow post ](https://stackoverflow.com/questions/15435530/using-mongodb-generated-ids-as-secret-data-eg-oauth-tokens) , so let's dig into it further. 
...
![](/images/blog/multi_cockpit_vulns/mongodb-docs-objectid.png)
...
_Right it looks like I have everything now to make a cool POC._
...
**` cockpit-blaster.py ` **
  ...
  def create_item(target: str, template: dict, payload_key: str, payload_value: str, api_key=***REDACTED*** -> requests.Response:
  ...
  def attempt_to_dump_obj(target: str, template: dict, model_name: str, id: str, payload_key: str, api_key=***REDACTED*** -> bool:
  obj_ref = {"_model": model_name, "_id": id}
  r = create_item(target, template, payload_key, obj_ref, api_key=***REDACTED***
  new_id = r.json()["_id"]
  ...
  # Get initial ID
  r = create_item(target, payload_template, payload_key, 'Test', api_key=***REDACTED***
  ...
  if attempt_to_dump_obj(target, payload_template, target_model, test_id, payload_key, api_key=***REDACTED***
  break
...
![](/images/blog/multi_cockpit_vulns/cockpit-idor-dump.gif)

##  How Cockpit CMS Patched This Vulnerability 

Artur from Cockpit CMS released a patch version to test **two days after my disclosure** ( _one of the fastest patches I have seen_ ) with the following change to the ` populate ` function. 

![](/images/blog/multi_cockpit_vulns/idor-patch.png)

Although this code change _does patch the vulnerability_ , there is still an edge case that could occur to make this exploited. Notice that if ` isset($proccess['user']['role']) ` is not set then the ` allowedModels ` verification is not executed and the vulnerability is still exploitable. 

_So when could this scenario could occur?_
...
![](/images/blog/multi_cockpit_vulns/norole1.png)

Results in the ` role ` **not** being set for the account. 

![](/images/blog/multi_cockpit_vulns/norole2.png)
...
![](/images/blog/multi_cockpit_vulns/cockpit-block.png)
...
![](/images/blog/multi_cockpit_vulns/idor-bypass.png)
...
It is also important to point that although the unauthorised access to models by exploiting the ` populate ` function was _patched_ , the **verifying the types of inputs to expected the type for a field has not been implemented** . This means you can still do things such as inject a JSON object in a text field that could lead to future vulnerabilities. 
...
![](/images/blog/multi_cockpit_vulns/where-is-csrf.png)

_Where on earth is the CSRF token validation for this file upload?_

![](/images/blog/multi_cockpit_vulns/gorilla-stare.gif)
...
_Let's harden my test server and see if I can still exploit these vulnerabilities._
...
![](/images/blog/multi_cockpit_vulns/after-saw.gif)

##  CSRF to XSS 

First I will look at the Assets API and see if I can leverage CSRF on Cockpit CMS to achieve XSS. Now some of you might of noticed I slightly _cheated_ with my rule and allowed uploading SVG files, that can be used to [ run arbitrary JavaScript when a victim views the file ](https://developer.mozilla.org/en-US/docs/Web/SVG) . Well it isn't cheating because I made up the game so I can chose my own rules. 

Anyway, it _should of not mattered_ since Cockpit CMS sanitised SVG files using ` \SVGSanitizer::clean ` in the following code from ` modules/Assets/bootstrap.php ` . 
...
This causes a _very very very small_ time window where the uploaded SVG file can be accessed from ` /storage/tmp ` on the website and ` \SVGSanitizer::clean ` has not completed sanitising the file. You can increase this time window by simply uploading a larger SVG file. 
...
The process of exploitation is: 
  ...
  2. On the second tab that is opened, wait for a little bit then _yeet_ the victim straight into uploaded SVG file in ` /storage/tmp ` before it is sanitised. 

The following POC GIF demonstrates doing this CSRF to XSS exploit and stealing the victim user's session cookie. 

![](/images/blog/multi_cockpit_vulns/cockpit-csrf-xss.gif)

##  CSRF to RCE 

So I had achieved leveraging the CSRF vulnerability to get XSS, but _I want some of that juicy RCE_ . 
...
![](/images/blog/multi_cockpit_vulns/finder-json.png)

_Did I give up there?_ **NOPE!**

I simply attempted the same request, but changed the ` Content-Type ` to ` application/x-www-form-urlencoded ` . 

![](/images/blog/multi_cockpit_vulns/json-csrf-bypass.png)
...
_Let's get into that POC since that is what everyone has probably skipped to._
...
![](/images/blog/multi_cockpit_vulns/cockpit-csrf-rce.gif)
...
One of my major concerns going through Cockpit CMS was that the default settings were **insecure** and could potentially lead to achieving **XSS or RCE** (as I demonstrated above even with _hardened_ settings). Normally I don't report insecure defaults, since the responsibility of setting secure configuration settings is on the user. 
...
![](/images/blog/multi_cockpit_vulns/cockpit-docs.png)

The only _'complete'_ documentation about configuring Cockpit CMS I could find was on [ this gitlab website by Zeraton ](https://zeraton.gitlab.io/cockpit-docs/guide/basics/configuration.html#introduction) that was last updated in **2019** . Therefore, I felt obliged to document some configuration settings for hardening your Cockpit CMS servers. 

##  Setting an Allow List for Assets API Uploads 

As previously mentioned, the Assets API is the _secure_ and _recommended_ alternative for file uploads on Cockpit CMS. However, by default it allows uploading any file with the only exception that ` .php ` , ` .phar ` and ` .phtml ` file extensions not being allowed. Instead, an allow list should be configured to restrict the allowed file extensions on Cockpit CMS. For an example, the following configuration setting only allows the upload of ` .png ` , ` .jpg ` and ` .jpeg ` file extensions using the Assets API. 

_Put in` config/config.php ` _
...
![](/images/blog/multi_cockpit_vulns/rmrf-Finder.gif)
...
![](/images/blog/multi_cockpit_vulns/default-key.png)
