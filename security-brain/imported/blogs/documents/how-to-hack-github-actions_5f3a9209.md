...
original_path: knowledge-inbox/blogs-incoming/2022-09-19_how-to-hack-github-actions.md
original_filename: 2022-09-19_how-to-hack-github-actions.md
...
- api-security
language: en
raw_sha256: 5f3a9209274529c1085cad349b40fd350ba0f5aa92f49fe5d1e044b58a1545b2
text_sha256: 9aa646b5adf7516d7f8e519b90680c059f878f7ff98a35cf9540219f96050646
...
- Original Path: knowledge-inbox/blogs-incoming/2022-09-19_how-to-hack-github-actions.md
- Source Type: markdown
- Detected Topics: supply-chain, sso, command-injection, otp, automation-abuse, api-security
- Ingested At: 2026-06-28T07:32:14Z
- Redactions Applied: True
- Raw SHA256: `5f3a9209274529c1085cad349b40fd350ba0f5aa92f49fe5d1e044b58a1545b2`
- Text SHA256: `9aa646b5adf7516d7f8e519b90680c059f878f7ff98a35cf9540219f96050646`
...
---
title: "How to hack Github Actions"
page_title: "GitHub - StackOverflowExcept1on/how-to-hack-github-actions: How to hack Github Actions ($500 bug bounty) · GitHub"
url: "https://github.com/StackOverflowExcept1on/how-to-hack-github-actions"
final_url: "https://github.com/StackOverflowExcept1on/how-to-hack-github-actions"
...
[![reward](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/reward.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/reward.png)
...
I made the private repository that is located at <https://github.com/StackOverflowExcept1on/how-to-hack-github-actions> and contains source code. If you can't access it, send me your GitHub username. So, I'll invite you.

As an alternative, I have attached the code of all projects, which maybe will useful at HackerOne.

### Prehistory

I got interested in writing GitHub Actions script for automation and came across an article: [Crypto-mining attack in my GitHub actions through Pull Request](https://dev.to/thibaultduponchelle/the-github-action-mining-attack-through-pull-request-2lmc) . It became interesting to me how exactly GitHub prevents such attacks. That's why I did this report.
...
[![01_provisioner](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/01_provisioner.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/01_provisioner.png)
...
[`.github/workflows/files.yml`](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/.github/workflows/files.yml)
...
After getting the executable, I used [dnSpyEx](https://github.com/dnSpyEx/dnSpy) to analyze it and found many interesting things

[![02_dnspy](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/02_dnspy.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/02_dnspy.png)
...
`provisioner.framework.dll` contains this classes, I also did screenshot of the decompiled code to show most important things:
  ...
  * `ActivityLogMonitorJob` \- **reports suspicious activity** [![03_ActivityLogMonitorJob](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/03_ActivityLogMonitorJob.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/03_ActivityLogMonitorJob.png)
  * `HostsFileMonitorJob` \- **checks that`C:\Windows\System32\Drivers\etc\hosts` not changed to block mining pools** [![04_HostsFileMonitorJob](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/04_HostsFileMonitorJob.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/04_HostsFileMonitorJob.png)
  * `MachineHealthMonitorJob` \- **sends some (CPU, RAM?) metrics over the network** [![05_MachineHealthMonitorJob](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/05_MachineHealthMonitorJob.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/05_MachineHealthMonitorJob.png)
  * `MachineInfoMonitorJob` \- not sure
  * `NetworkHealthMonitorJob` \- checks that network is reachable
  * `ProcessMonitorJob` \- **checks if process has mining-related activity (arguments, process name)** [![06_ProcessMonitorJob](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/06_ProcessMonitorJob.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/06_ProcessMonitorJob.png)
  * `ProvjobdMonitorJob` \- runs some golang executable file
  * `SuspiciousFilesMonitorJob` \- **recursively scans for malware/miners at the VM** [![07_SuspiciousFilesMonitorJob](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/07_SuspiciousFilesMonitorJob.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/07_SuspiciousFilesMonitorJob.png)
...
[![explained_01_how_to_inject](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/explained/01_how_to_inject.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/explained/01_how_to_inject.png)

At first need to perform shellcode injection. You can read about it here: [Executing Shellcode in Remote Process](https://www.ired.team/offensive-security/code-injection-process-injection/process-injection)
...
I put this code into [`projects/cpp/app/codeinjector/src/main.cpp`](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/projects/cpp/app/codeinjector/src/main.cpp). So, you can see how it was done

This injection is still not enough to get to c# code. I spent about a week while doing reverse engineering work dotnet internals and found this article: [Write a custom .NET host to control the .NET runtime from your native code](https://docs.microsoft.com/en-us/dotnet/core/tutorials/netcore-hosting)
...
Well, I wrote module that is called `bootstrapper`. You again can see source code if you need at [`projects/cpp/app/bootstrapper/src/main.cpp`](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/projects/cpp/app/bootstrapper/src/main.cpp)
...
So, I used IDA Pro to locate where `fx_muxer_t::get_active_host_context()` is used. It's used in `hostfxr_get_runtime_properties` which is also exported! It allows me to get address of this function [![hostfxr_aslr_bypass_01](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/hostfxr_aslr_bypass/01_hostfxr_get_runtime_properties.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/hostfxr_aslr_bypass/01_hostfxr_get_runtime_properties.png)
...
In x86_64 assembler each call instruction starts from opcode `0xE8`: see <https://www.felixcloutier.com/x86/call>
...
[![02_hostfxr_aslr_bypass](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/hostfxr_aslr_bypass/02_right_address.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/hostfxr_aslr_bypass/02_right_address.png)

Now imagine if we write a program that does the same at runtime! It will just run through each byte and read the machine code until it finds 0xE8, and then calculate the address [![03_hostfxr_aslr_bypass](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/hostfxr_aslr_bypass/03_how_to_find_call.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/hostfxr_aslr_bypass/03_how_to_find_call.png)
...
After successful injection I can use [Harmony](https://github.com/pardeike/Harmony) library to attach my code to any function of `provisioner.exe`. I can insert my custom code at the beginning of any C# function, then cancel execution of original function. Also, Harmony library allows performing custom code at end of function to replace its result.

[![08_patch_logic](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/08_patch_logic.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/08_patch_logic.png)

### Debugging Virtual Environment Provisioner using TCP-server

[![explained_02_how_server_works](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/explained/02_how_server_works.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/explained/02_how_server_works.png)
...
using code like this
  ...
  public class ***REDACTED-SUSPECT-TOKEN***  {
  [HarmonyPrefix]
...
[![09_first_inject](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/09_first_inject.jpeg)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/09_first_inject.jpeg)
...
[![10_suspicious_files_checker](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/10_suspicious_files_checker.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/10_suspicious_files_checker.png)
...
[![11_process_checking_job_init](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/11_process_checking_job_init.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/11_process_checking_job_init.png)
...
[![12_script_task_validator](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/12_script_task_validator.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/12_script_task_validator.png)
...
[![13_xmr_address_to_trigger](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/13_xmr_address_to_trigger.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/13_xmr_address_to_trigger.png)
...
[![14_script_task_validator_check](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/14_script_task_validator_check.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/14_script_task_validator_check.png)
...
[![15_process_monitor_job_calls_validator](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/15_process_monitor_job_calls_validator.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/15_process_monitor_job_calls_validator.png)
...
You can see full code at [`projects/csharp/patcher/patcher/Main.cs`](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/projects/csharp/patcher/patcher/Main.cs)

### How to run PoC on GitHub Actions VM?

I made a complete project which has the file [`.github/workflows/build.yml`](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/.github/workflows/build.yml) to do this. You can upload it to GitHub, then it will automatically run action that's builds payload from sources, runs local python server and exits after 15 minutes.

If you want to repeat this, you can do as in the screenshot: [![16_how_to_build_and_test_poc](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/16_how_to_build_and_test_poc.png)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/16_how_to_build_and_test_poc.png)
...
[![cat](/StackOverflowExcept1on/how-to-hack-github-actions/raw/master/assets/cat.jpg)](/StackOverflowExcept1on/how-to-hack-github-actions/blob/master/assets/cat.jpg)
