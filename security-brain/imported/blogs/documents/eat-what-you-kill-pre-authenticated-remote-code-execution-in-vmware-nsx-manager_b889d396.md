...
original_path: knowledge-inbox/blogs-incoming/2022-10-25_eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager.md
original_filename: 2022-10-25_eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager.md
title: 'Eat What You Kill :: Pre-authenticated Remote Code Execution in VMWare NSX
  Manager'
category: documents
detected_topics:
- command-injection
...
- csrf
language: en
raw_sha256: b889d3967cb2ce01ec29ffd00c2ae6180ac7a114a5b008485d252c79815ddb32
text_sha256: 74c280e9b9e418743b20d97d667e05335e17ad49393c7f2dfaacea030ab95797
...
- Original Path: knowledge-inbox/blogs-incoming/2022-10-25_eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager.md
- Source Type: markdown
- Detected Topics: command-injection, oauth, sso, jwt, otp, csrf
- Ingested At: 2026-06-28T07:32:15Z
- Redactions Applied: True
- Raw SHA256: `b889d3967cb2ce01ec29ffd00c2ae6180ac7a114a5b008485d252c79815ddb32`
- Text SHA256: `74c280e9b9e418743b20d97d667e05335e17ad49393c7f2dfaacea030ab95797`
...
final_url: "https://srcincite.io/blog/2022/10/25/eat-what-you-kill-pre-authenticated-rce-in-vmware-nsx-manager.html"
...
![](/assets/images/eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager/logo.png)
...
![](/assets/images/eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager/xstream.png)
...
In addition to user-defined types like _Person_ , `XStream` recognizes core Java types out of the box. For example, `XStream` can read a _Map_ from XML:
...
![](/assets/images/eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager/MethodClosure.png)
...
The `doCall` method of `MethodClosure` class can be called by using the `call` method of the parent class, why this much pain? well if you remember the `doCall` in `MethodClosure` has the `protected` access modifier which means the method can be accessed within the class and by classes derived from that class. As you can see at _[3]_ the `call` function is invoking the `doCall` method at _[4]_ from the `getMetaClass()` which is the `MethodClosure` instance.
...
![](/assets/images/eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager/Expando.png)
...
The code at _[1]_ will get the property called `hashCode` and cast it to a `Closure` type at _[2]_ and finally call `call` on it at _[3]_. The question now remains, how are we going to automatically call the `hashCode` on our `Expando` object? `hashCode` is called when objects keys are compared and we can create a `HashMap` to put the `Expando` object in as one of its members so that when the `hashMap` is getting instantiated during the unmarshalling, the `hashCode` method will be called.
...
At _[4]_ `hashCode` is called, which means we can finally get code injection upon object reconstruction:
...
![Courtesy of @lystena](/assets/images/eat-what-you-kill-pre-authenticated-remote-code-execution-in-vmware-nsx-manager/popthyshell.png)

_Image Courtesy of[@lystena](https://twitter.com/lystena)_
  ...
  SHA1: ***REDACTED-SUSPECT-TOKEN***  Found by: Sina Kheirkhah of MDSec and Steven Seeley of Source Incite
  """
