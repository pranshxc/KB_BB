...
original_path: knowledge-inbox/blogs-incoming/2021-05-30_metadata-service-mitm-allows-root-privilege-escalation-eks-gke.md
original_filename: 2021-05-30_metadata-service-mitm-allows-root-privilege-escalation-eks-gke.md
...
- otp
language: en
raw_sha256: 1fb2480a783680b58ead9c130f5121267228483bfc7a9724a3de7fcf6027ae95
text_sha256: e5b5485198d45a2ee2c6a629373fca7345d02a0669ca0d83358b977e34246f27
...
- Original Path: knowledge-inbox/blogs-incoming/2021-05-30_metadata-service-mitm-allows-root-privilege-escalation-eks-gke.md
- Source Type: markdown
- Detected Topics: cloud-security, access-control, ssrf, xss, command-injection, otp
- Ingested At: 2026-06-28T07:32:06Z
- Redactions Applied: False
- Raw SHA256: `1fb2480a783680b58ead9c130f5121267228483bfc7a9724a3de7fcf6027ae95`
- Text SHA256: `e5b5485198d45a2ee2c6a629373fca7345d02a0669ca0d83358b977e34246f27`
...
---
title: "Metadata service MITM allows root privilege escalation (EKS / GKE)"
page_title: "blog/Metadata_MITM_root_EKS_GKE at main · champtar/blog · GitHub"
url: "https://github.com/champtar/blog/tree/main/Metadata_MITM_root_EKS_GKE"
final_url: "https://github.com/champtar/blog/tree/main/Metadata_MITM_root_EKS_GKE"
...
After finding that having containers with `CAP_NET_RAW` could allow you to launch [Host MITM attack via IPv6 rogue router advertisements](/champtar/blog/blob/main/IPv6_RA_MITM/README.md), on top of classic ARP spoofing when using bridge CNI, i went ahead and looked for more fun network attacks.

`CAP_NET_RAW` Linux capability allows you open raw sockets to listen on all traffic in the network namespace (tcpdump), but also to send any type of packets. `CAP_NET_RAW` is enabled by default in Docker/Containerd, but not in [cri-o since 1.18](https://github.com/cri-o/cri-o/commit/63b9f4ec986da267a18f73d4b1ef13282d103e00) only to allow `ping` to work. It's a bad default that should be replaced by the usage of the `net.ipv4.ping_group_range` sysctls.
...
Another way to get MITM is to use [K8S CVE-2020-8554](/champtar/blog/blob/main/K8S_MITM_LoadBalancer_ExternalIPs/README.md).
...
An attacker gaining access to a `hostNetwork=true` container with `CAP_NET_RAW` capability can listen to all the traffic going through the host and inject arbitrary traffic, allowing to tamper with most unencrypted traffic (HTTP, DNS, DHCP, ...), and disrupt encrypted traffic. In GKE the host queries the metadata service at <http://169.254.169.254> to get information, including the authorized ssh keys. By manipulating the metadata service responses, injecting our own ssh key, we gain root privilege on the host.
  ...
  3. Copy [metadatascapy.py script](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/GKE/metadatascapy.py))
...
In the past, using L2 socket on lo interface was not working so [Scapy FAQ recommend to use L3](https://scapy.readthedocs.io/en/latest/troubleshooting.html#i-can-t-ping-127-0-0-1-scapy-does-not-work-with-127-0-0-1-or-on-the-loopback-interface). Also GKE nodes were already using `net.ipv4.conf.all.rp_filter = 1` sysctl effectively blocking L2. To block L3 Google team just added the following iptables rules on GKE nodes
...
The [eic_curl_authorized_keys script (before fix)](https://github.com/aws/aws-ec2-instance-connect-config/blob/47de50509ed43f0c294513841739afb059d5900e/src/bin/eic_curl_authorized_keys) queries the metadata service at <http://169.254.169.254> One of the responses is a signed list of ssh keys with the instance name and an expiration timestamp like this:
...
On AWS EKS, an attacker able to get arbitrary code execution as root in a hostNetwork pod can use the `CAP_NET_RAW` capability to monitor and inject packets on the instance.
...
This POC shows how easy we can go from hostNetwork to the node. It uses Golang (gopacket) as scapy was often slower to inject packets than the metadata service to respond, making the attack unreliable.
  ...
  1. [cert0.pem](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/cert0.pem): CN=managed-ssh-signer.us-east-2.champetier.net, expired Jan 10 2021
  2. [privkey.pem](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/privkey.pem): the private key of the cert
  3. [cert1.pem](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/cert1.pem): CN=Let's Encrypt Authority X3
  4. [cert2.pem](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/cert2.pem): CN=DST Root CA X3
  5. [go.mod](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/go.mod), [go.sum](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/go.sum), [mitmmeta.go](/champtar/blog/blob/main/Metadata_MITM_root_EKS_GKE/EKS/mitmmeta.go): The Golang POC
...
Searching in [Certificate Transparency logs](https://censys.io/certificates?q=parsed.names%3A%2Fmanaged-ssh-signer.*%2F), it seems I'm the only one having created such cert.
