...
original_path: knowledge-inbox/blogs-incoming/2019-03-01_bypassing-a-restrictive-js-sandbox.md
original_filename: 2019-03-01_bypassing-a-restrictive-js-sandbox.md
...
- automation-abuse
language: en
raw_sha256: 1990b3134fb36f54041fe97726a3128cea15ae612ba487c0994070f489f43724
text_sha256: ee7153902fa5387a928a4e725115d863cfd2cc2c8b8a06c9f03fe62679f02aaf
...
- Original Path: knowledge-inbox/blogs-incoming/2019-03-01_bypassing-a-restrictive-js-sandbox.md
- Source Type: markdown
- Detected Topics: command-injection, supply-chain, sso, xss, path-traversal, automation-abuse
- Ingested At: 2026-06-28T07:31:58Z
- Redactions Applied: False
- Raw SHA256: `1990b3134fb36f54041fe97726a3128cea15ae612ba487c0994070f489f43724`
- Text SHA256: `ee7153902fa5387a928a4e725115d863cfd2cc2c8b8a06c9f03fe62679f02aaf`
...
---
title: "Bypassing a restrictive JS sandbox"
page_title: "Bypassing a restrictive JS sandbox | Licencia para Hackear"
url: "https://licenciaparahackear.github.io/en/posts/bypassing-a-restrictive-js-sandbox/"
final_url: "https://licenciaparahackear.github.io/en/posts/bypassing-a-restrictive-js-sandbox/"
...
[Español](../../../posts/bypasseando-una-sandbox-de-js-restrictiva/)
...
Finally, by analyzing the app error messages I concluded that they were using [static-eval](https://github.com/substack/static-eval), a not very known library (but written by [substack](https://twitter.com/substack), somebody well known in the NodeJS community). Even if the original purpose of the library wasn't to be used as a sandbox (I still don't understand what it was created for), its documentation suggests that. In the case of the site I was testing, it certainly was being used as a sandbox.

### Breaking static-eval

The idea of static-eval is to use the [esprima](https://github.com/jquery/esprima/) library to parse the JS expression and convert it to an [AST (Abstract Syntax Tree)](https://en.wikipedia.org/wiki/Abstract_syntax_tree). Given this AST and an object with the variables I want to be available inside the sandbox, it tries to evaluate the expression. If it finds something strange, the function fails and my code isn't executed. At first I was a bit demotivated because of this, since I realized that the sandbox system was very restrictive with what it accepted. I wasn't even able to use a `for` or `while` statement inside my expression, so doing something that required an iterative algorithm was almost impossible. Anyway, I kept trying to find a bug in it.

I did not find any bug at first sight, so I looked at the commits and pull requests of the [static-eval GitHub project](https://github.com/substack/static-eval). I found that the [pull request #18](https://github.com/substack/static-eval/pull/18) fixed two bugs that allowed a sandbox escape in the library, exactly what I was looking for. I also found [a blog post of the pull request author](https://maustin.net/articles/2017-10/static_eval) that explained this vulnerabilities in depth. I immediately tried using this techniques in the site I was testing, but unfortunately to me, they were using a newer static-eval version that already patched this vulns. However, knowing that somebody has already been able to break this library made me more confident so I kept looking for new ways to bypass it.
...
The first vuln used the [function constructor](https://developer.mozilla.org/es/docs/Web/JavaScript/Referencia/Objetos_globales/Function) to make a malicious function. This technique is frequently used to bypass sandboxes. For example, most of [the ways to bypass the angular.js sandbox to get an XSS](https://portswigger.net/blog/xss-without-html-client-side-template-injection-with-angularjs) use payloads that end up accessing and calling the function constructor. It was also used to [bypass libraries similar to static-eval, like vm2](https://github.com/patriksimek/vm2/issues/32). The following expression shows the existence of the vulnerability by printing the system environment variables (this shouldn't be possible because the sandbox should block it):
...
![Result of executing the previous payload](../../../images/static-eval/constructor-bypass.png)
...
![Result of executing the payload that runs the id command in the system](../../../images/static-eval/constructor-execsync.png)
...
This code takes the AST node that defines the function, iterates over each of its parameters whose type is `Identifier`, takes its name and sets to null the attribute of `vars` with that name. Even if the code looks correct, it has a very common bug: it doesn't cover all possible cases. What would happen if an argument is something strange and its type isn't `Identifier`? instead of doing something sane and saying _"I don't know what this is, so I'll block the entire function"_ (like in a whitelist), it will ignore that argument and continue with the rest (like a blacklist). This means that if I make a node representing a function argument have a type different from `Identifier`, the value of the variable with that name won't be overwritten, so it would use the initial value. At this time I was pretty confident that I found something important. I only needed to find how to set the `key.type` to something different from `Identifier`.

As I commented before, static-eval uses [the esprima library](https://github.com/jquery/esprima/) to parse the code we give to it. According to its documentation, esprima is a parser that fully supports the [ECMAScript standard](https://www.ecma-international.org/ecma-262/7.0/). ECMAScript is something like a dialect of JavaScript with more features, that makes its syntax more comfortable to the user1.

One feature that was added to ECMAScript is [function parameter destructuring](https://simonsmith.io/destructuring-objects-as-function-parameters-in-es6/). With this feature, the following JS code is now valid:
...
If we see the AST generated by esprima (I did it by using [this tool](http://esprima.org/demo/parse.html)), we will have a very satisfactory result:

![Result of parsing the function using parameter destructuring](../../../images/static-eval/esprima-online-parser-en.png)
...
![Final working exploit](../../../images/static-eval/exploit-final.png)
...
To give some context, the site wasn't using static-eval directly. It was using it through the [jsonpath](https://github.com/dchester/jsonpath) npm library. JSONPath is a query language with the same purpose as XPATH but made for JSON documents instead of XML ones. It was initially published in 2007 in [this article](https://goessner.net/articles/JsonPath/).
...
![](../../../images/static-eval/npm-install.jpeg)
...
I want to mention again the [great previous work done by Matt Austin about static-eval](https://maustin.net/articles/2017-10/static_eval). Without this material, maybe I wouldn't have found this new vulnerability.
...
## Extra: Cronology of the vuln
  ...
  * 01/02/19 - Report of the vulnerability submitted both to the NodeJS security team and to the static-eval mantainer. You can read the original report [here](../../../posts/static-eval-sandbox-escape-original-writeup/)
  ...
  * 02/14/19 - [Advisory officially published in the nmpjs site](https://www.npmjs.com/advisories/758)
  * 02/15/19 - [The library was fixed](https://github.com/browserify/static-eval/pull/21) and a new version of it was released
  * 02/18/19 - The library's README file was updated to add a [disclaimer saying that the library shouldn't be used as a sandbox](https://github.com/browserify/static-eval/pull/22/files)
  * 02/26/19 - [A new fix](https://github.com/browserify/static-eval/pull/23) was applied to the library because my original fix had a bug and static-eval was still vulnerable
...
* * *
  ...
  * [bug bounty](../../categories/bug-bounty/)
  * [bypass](../../categories/bypass/)
  * [javascript](../../categories/javascript/)
  * [jsonpath](../../categories/jsonpath/)
  * [nodejs](../../categories/nodejs/)
  * [remote code execution](../../categories/remote-code-execution/)
  * [sandbox](../../categories/sandbox/)

  * [Previous post](../licencia-para-hackear-esta-de-vuelta/ "Licencia para Hackear está de vuelta!")
  * [Next post](../invitacion-a-nerdearla/ "Invitación a nerdear.la")
