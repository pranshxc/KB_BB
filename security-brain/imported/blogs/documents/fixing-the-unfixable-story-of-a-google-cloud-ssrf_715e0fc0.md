...
original_path: knowledge-inbox/blogs-incoming/2021-12-31_fixing-the-unfixable-story-of-a-google-cloud-ssrf.md
original_filename: 2021-12-31_fixing-the-unfixable-story-of-a-google-cloud-ssrf.md
...
- command-injection
language: en
raw_sha256: 715e0fc0ae2b28afe8d4519319a5a84a0bd2823fe65cd8bc207aee3b0fb0b64e
text_sha256: 5d32fbd5c245ac3aef0a61d4c491dcc201025a6c14114405539db680450c9162
...
- Original Path: knowledge-inbox/blogs-incoming/2021-12-31_fixing-the-unfixable-story-of-a-google-cloud-ssrf.md
- Source Type: markdown
- Detected Topics: ssrf, rate-limit, cloud-security, oauth, access-control, command-injection
- Ingested At: 2026-06-28T07:32:09Z
- Redactions Applied: True
- Raw SHA256: `715e0fc0ae2b28afe8d4519319a5a84a0bd2823fe65cd8bc207aee3b0fb0b64e`
- Text SHA256: `5d32fbd5c245ac3aef0a61d4c491dcc201025a6c14114405539db680450c9162`
...
---
title: "Fixing the Unfixable: Story of a Google Cloud SSRF"
page_title: "Fixing the Unfixable: Story of a Google Cloud SSRF - bugs.xdavidhu.me"
url: "https://bugs.xdavidhu.me/google/2021/12/31/fixing-the-unfixable-story-of-a-google-cloud-ssrf/"
final_url: "https://bugs.xdavidhu.me/google/2021/12/31/fixing-the-unfixable-story-of-a-google-cloud-ssrf/"
...
After playing around with the `/proxy` endpoint by sending multiple requests trying to break the whitelist, I tried using the _backslash-trick_ from my previous writeup titled [“The unexpected Google wide domain check bypass”](/google/2020/03/08/the-unexpected-google-wide-domain-check-bypass/).
...
So I tried using the _backslash-trick_ on `cxl-services` as well, hoping that the whitelist validator and the actual request sending logic might parse the same URL differently:
...
response:
  ...
  X-Cloud-Trace-Context: ***REDACTED-SUSPECT-TOKEN***  Date: Mon, 23 Mar 2020 21:28:07 GMT
...
Setting my simple [Python HTTPS server](https://gist.github.com/xdavidhu/9491a9918b4dfbaffb789a8b9f2d1672) into `--verbose` mode, and making `cxl-services` request it once again allowed me to see the whole request going to my webserver, including all of the headers:
  ...
  authorization: Bearer ***REDACTED***(GNaGILMgosUyDOSxSAp0AGTqC10692v_K6_B39nlezaV5ntV3MdJ-ZcipXA3zt1CpbgkANgNRFrshzCqzc9Vy_AimSdan8F-ZngZec081 
...
_What can we use this access token for?_
...
We might wonder, does this token/identity have access to some GCP resources (VMs, storage buckets, etc.) other than the `jobs` API? In the Amazon AWS universe, we would have a much easier time here. Unfortunately in Google Cloud, you can’t ask the question _“what do I have access to?”_. You can only go to resources one-by-one, and ask _“do I have access to**this?** “_. Dylan Ayrey and Allison Donovan have made [an awesome talk](https://www.youtube.com/watch?v=Ml09R38jpok) about this behavior.
...
_A warning: Be careful and document your actions if you decide on using stolen credentials. There is a line in bug bounties which we shouldn’t cross. I could have reported the issue as-is, but I wanted to look around to prove that getting access to this identity is indeed impactful. I asked for permission from the Google team before performing any data-modifying actions._

Calling the [`projects.list`](https://cloud.google.com/resource-manager/reference/rest/v1/projects/list) method of the Resource Manager API, I found 4 GCP projects that this identity had _some level of_ access to:
...
_(psst: on Google VRP, you don’t have to wait until your issue moves into`fixed` status. if you find that the code has changed, but you can still exploit it, write a comment on the original ticket and you might get another reward!)_
...
Once again, the Google VRP panel rewarded this second bypass as well. So, check your fixes.. _of your fixes!_

**Will _you_ be the one to bypass it for the 3rd time and get $3133.7?**
