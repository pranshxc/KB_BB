...
original_path: knowledge-inbox/blogs-incoming/2024-09-02_part-2-from-byovdll-to-arbitrary-code-execution-in-lsass.md
original_filename: 2024-09-02_part-2-from-byovdll-to-arbitrary-code-execution-in-lsass.md
...
- automation-abuse
language: en
raw_sha256: 918dfbc4d64214207365eddc1a02b2420f29fba05e6decef2824ecd18446c923
text_sha256: 0e2a5dc9623067af6ffabe06b2447823cf83b7a46109a8f0f850e522ee093287
...
- Original Path: knowledge-inbox/blogs-incoming/2024-09-02_part-2-from-byovdll-to-arbitrary-code-execution-in-lsass.md
- Source Type: markdown
- Detected Topics: race-condition, sso, idor, command-injection, rate-limit, automation-abuse
- Ingested At: 2026-06-28T07:32:37Z
- Redactions Applied: False
- Raw SHA256: `918dfbc4d64214207365eddc1a02b2420f29fba05e6decef2824ecd18446c923`
- Text SHA256: `0e2a5dc9623067af6ffabe06b2447823cf83b7a46109a8f0f850e522ee093287`
...
---
title: "Part 2: From Byovdll To Arbitrary Code Execution In Lsass"
page_title: "Ghost in the PPL Part 2: From BYOVDLL to Arbitrary Code Execution in LSASS – SCRT Team Blog"
url: "https://blog.scrt.ch/2024/08/15/ghost-in-the-ppl-part-2-from-byovdll-to-arbitrary-code-execution-in-lsass/"
final_url: "https://blog.scrt.ch/2024/08/15/ghost-in-the-ppl-part-2-from-byovdll-to-arbitrary-code-execution-in-lsass/"
...
[![](/wp-content/uploads/2024/05/ida-srvcryptfreekey-2.png)](/wp-content/uploads/2024/05/ida-srvcryptfreekey-2.png)IDA – Pseudo-source code showing CVE-2023-28229
...
[![](/wp-content/uploads/2024/05/ida-graph-call-uaf-1.png)](/wp-content/uploads/2024/05/ida-graph-call-uaf-1.png)IDA – Graph view showing the CALL instruction
...
Even with these constraints, it would still be quite easy to write an Object Directory handle to the global variable `LdrpKnownDllDirectoryHandle`, so that we can later load unsigned DLLs, as I did in my previous [PPLmedic](/2023/03/17/bypassing-ppl-in-userland-again/) exploit. (Un)fortunately, this is no longer possible because this variable was moved to the Mutable Read Only Heap Section (`.mrdata`), which cannot be modified once the process is fully initialized. To work around this protection, the access rights of the memory area would have to be updated first.
...
Using PowerShell, and the script `[Get-PEHeader.ps1](https://github.com/mattifestation/PIC_Bindshell/blob/master/PIC_Bindshell/Get-PEHeader.ps1)`, I automated the parsing of all the modules loaded by LSASS, and found a total of 5225 unique imported APIs.
  ...
  $AllImports = @(); foreach ($m in (Get-Content .\lsass_loaded_modules.txt)) { if ($m -notlike "*.dll") { continue }; $Header = Get-PEHeader "C:\Windows\System32\$m"; $Header.Imports | % { $AllImports += "$($_.ModuleName):$($_.FunctionName)"  }; }
  ...
  foreach ($m in (Get-Content .\lsass_loaded_modules.txt)) { if ($m -notlike "*.dll") { continue }; $Header = Get-PEHeader "C:\Windows\System32\$m"; $Header.Imports | % { "$($m),$($_.ModuleName),$($_.FunctionName)" | Out-File .\lsass_loaded_modules_functions.txt -Append } }
...
If this [technique](https://www.deepinstinct.com/blog/lsass-memory-dumps-are-stealthier-than-ever-before-part-2) works, it’s a quick win because it only requires a process handle to be passed as the first parameter, the second parameter (_i.e._ the process exit code) being irrelevant.
...
[![](/wp-content/uploads/2024/06/system-informer-lsass-open-handles-1.png)](/wp-content/uploads/2024/06/system-informer-lsass-open-handles-1.png)System Informer – List of service processes opened by LSASS
...
[![](/wp-content/uploads/2024/06/system-informer-lsass-open-handles-2.png)](/wp-content/uploads/2024/06/system-informer-lsass-open-handles-2.png)System Informer – List of user processes opened by LSASS
...
[![](/wp-content/uploads/2024/06/api-monitor-lsasrv-ntopenprocess.png)](/wp-content/uploads/2024/06/api-monitor-lsasrv-ntopenprocess.png)API Monitor showing a call to `NtOpenProcess` within LSASS
...
[![](/wp-content/uploads/2024/06/api-monitor-lsasrv-ntopenprocess-call-stack.png)](/wp-content/uploads/2024/06/api-monitor-lsasrv-ntopenprocess-call-stack.png)Call stack leading to call to `NtOpenProcess`
...
[![](/wp-content/uploads/2024/06/ghidra-lsasrv-ntopenprocess-disas.png)](/wp-content/uploads/2024/06/ghidra-lsasrv-ntopenprocess-disas.png)Ghidra – Call to `NtOpenProcess` in `lsasrv.dll`
...
[![](/wp-content/uploads/2024/06/ghidra-lsasrv-ntopenprocess-pseudo-code.png)](/wp-content/uploads/2024/06/ghidra-lsasrv-ntopenprocess-pseudo-code.png)Ghidra – `NtOpenProcess` invoked by `LsapOpenCaller`
...
[![](/wp-content/uploads/2024/06/rpcview-sspi-interface-1024x715.png)](/wp-content/uploads/2024/06/rpcview-sspi-interface.png)RpcView – SSPI RPC interface
...
[![](/wp-content/uploads/2024/06/rpc-message-structure.drawio.png)](/wp-content/uploads/2024/06/rpc-message-structure.drawio.png)Structures and data required by `NdrServerCallAll`
...
[![](/wp-content/uploads/2024/06/poc_ndrservercall_outputdebugstring.png)](/wp-content/uploads/2024/06/poc_ndrservercall_outputdebugstring.png)Calling `OutputDebugStringW` through `NdrServerCallAll`
...
[![](/wp-content/uploads/2024/06/poc_ndrservercall_duplicatehandle.png)](/wp-content/uploads/2024/06/poc_ndrservercall_duplicatehandle.png)Calling `DuplicateHandle` through `NdrServerCallAll`
...
[![](/wp-content/uploads/2024/06/fake-object-provider.drawio.png)](/wp-content/uploads/2024/06/fake-object-provider.drawio.png)RPC and NDR structures packed in a fake Key Provider object
...
[![](/wp-content/uploads/2024/06/exploit-duplicatehandle-test-success.png)](/wp-content/uploads/2024/06/exploit-duplicatehandle-test-success.png)`DuplicateHandle` called through `NdrServerCallAll` within LSASS
...
Posted on [August 15, 2024September 5, 2024](/2024/08/15/ghost-in-the-ppl-part-2-from-byovdll-to-arbitrary-code-execution-in-lsass/)Author [Cl├®ment Labro](/author/cla/)Categories [Exploit](/category/exploit/), [Research](/category/research/)
