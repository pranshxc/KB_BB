...
original_path: knowledge-inbox/blogs-incoming/2022-02-18_rce-in-github-desktop-294.md
original_filename: 2022-02-18_rce-in-github-desktop-294.md
...
- mobile-security
language: en
raw_sha256: 5f90df1b6a2434ac70faf47ef82ee00d43492a6baefc178b8ac81ec412499e0d
text_sha256: db27b1ed702fa597b82f28b39730c12c4e3c9051989321478ee7e0c6ffde530e
...
- Original Path: knowledge-inbox/blogs-incoming/2022-02-18_rce-in-github-desktop-294.md
- Source Type: markdown
- Detected Topics: command-injection, automation-abuse, mobile-security
- Ingested At: 2026-06-28T07:32:10Z
- Redactions Applied: False
- Raw SHA256: `5f90df1b6a2434ac70faf47ef82ee00d43492a6baefc178b8ac81ec412499e0d`
- Text SHA256: `db27b1ed702fa597b82f28b39730c12c4e3c9051989321478ee7e0c6ffde530e`
...
---
title: "RCE in GitHub Desktop < 2.9.4"
page_title: "write-ups/rce-github-desktop-2.9.3 at main · Metnew/write-ups · GitHub"
url: "https://github.com/Metnew/write-ups/tree/main/rce-github-desktop-2.9.3"
final_url: "https://github.com/Metnew/write-ups/tree/main/rce-github-desktop-2.9.3"
...
[![](/Metnew/write-ups/raw/main/rce-github-desktop-2.9.3/github_desktop_openlocalrepo_safari.gif)](/Metnew/write-ups/blob/main/rce-github-desktop-2.9.3/github_desktop_openlocalrepo_safari.gif)
...
It's obvious why `git` was [selected by Google](/Metnew/write-ups/blob/main/rce-github-desktop-2.9.3/news/google-commits-to-solving-security-problems-in-open-source-projects-like-git-and-laravel) for a security audit as a top-priority open-source project git.
...
Unlike `openrepo` URI command, `openlocalrepo` doesn't ask the user to confirm local repository import. `github-mac://openlocalrepo/<path_to_local_repo>` command immediately imports the specified local repository into GitHub Desktop app.
...
> Make sure to replace the test payload To confirm the config is poisoned, run `git status` inside the repo

This repo has [git filters](https://git-scm.com/book/en/v2/Customizing-Git-Git-Attributes) that GitHub Desktop will execute as soon as the repo is imported into the app. When Desktop imports the repo, it'll run `any` filter for `.gitattributes` because this file hasn't been committed yet.
...
> it's possible to guess OS/browser from User-Agent it's possible to uncover OS username by navigating user browser to `ssh://` link (if the user confirms -> an SSH session will start with the current username). There are other esoteric methods like stealing the OS username by sending an HTML file to the target (and disclosing the full path using JS later).

**Safari:`"Open 'safe' files"` isn't that safe**

**By default** , Safari has **"Open 'safe' files after downloading"** option enabled. Thus, running `location.href = "./payload.zip"` makes Safari unzipping the archive to `~/Downloads/evil` **without any user interaction**.

**Delivering malicious files with`smb://`**

If it's macOS or Windows, we can drop the folder with `location.href="smb://publicshare"` In macOS, it requires not more than a single click from the targeted user.
