...
original_path: knowledge-inbox/blogs-incoming/2022-03-28_pwning-microsoft-azure-defender-for-iot-multiple-flaws-allow-remote-code-executi.md
original_filename: 2022-03-28_pwning-microsoft-azure-defender-for-iot-multiple-flaws-allow-remote-code-executi.md
title: Pwning Microsoft Azure Defender for IoT | Multiple Flaws Allow Remote Code
  Execution for All
category: documents
detected_topics:
- command-injection
...
- access-control
language: en
raw_sha256: 8be23b03a780ce824f65d119c79b6d9d98d4498c943cc8bdbca4e21cc6d7b2c3
text_sha256: ae2d1d20eca055a93631d5efb12ab6aec56a93e157ebd309ffb1253ec784c105
...
- Original Path: knowledge-inbox/blogs-incoming/2022-03-28_pwning-microsoft-azure-defender-for-iot-multiple-flaws-allow-remote-code-executi.md
- Source Type: markdown
- Detected Topics: command-injection, cloud-security, race-condition, supply-chain, sso, access-control
- Ingested At: 2026-06-28T07:32:10Z
- Redactions Applied: True
- Raw SHA256: `8be23b03a780ce824f65d119c79b6d9d98d4498c943cc8bdbca4e21cc6d7b2c3`
- Text SHA256: `ae2d1d20eca055a93631d5efb12ab6aec56a93e157ebd309ffb1253ec784c105`
...
page_title: "IoT Security Exposed: Unveiling Azure Defender Flaws"
url: "https://www.sentinelone.com/labs/pwning-microsoft-azure-defender-for-iot-multiple-flaws-allow-remote-code-execution-for-all/"
final_url: "https://www.sentinelone.com/labs/pwning-microsoft-azure-defender-for-iot-multiple-flaws-allow-remote-code-execution-for-all/"
...
[Kasif Dekel](https://www.sentinelone.com/blog/author/kasifd/) /  [ March 28, 2022 ](https://www.sentinelone.com/blog/2022/03/)
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image2-2.jpg)Source: [Microsoft Azure Defender for IoT architecture](https://docs.microsoft.com/en-us/azure/defender-for-iot/organizations/architecture)
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image5-1.jpg)
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image6-1.jpg)Route controllers that do not require authentication
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image4-1.jpg)
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image1-2.jpg)
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/S1-FullAnimation.v4.more-1.gif)
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image7-1.jpg)
...
For that matter, we can create a new trial Azure account and generate a recovery file using that interface mentioned above. The secret identifier is irrelevant and may contain garbage.
  ...
  * `IotDefenderSigningCertificate.pem` – a self-signed certificate. It can be generated by the following command: 
  ...
  openssl  req  -x509  -nodes  -newkey rsa:2048  -keyout key.pem  -out IotDefenderSigningCertificate.pem  -subj "/C=DE/ST=NRW/L=Berlin/O=My Inc/OU=ALEG/CN=www.example.com/[[email protected]](/cdn-cgi/l/email-protection)"
...
The _benign.zip_ file is the ZIP file obtained from the Azure portal, as described above and the _malicious.zip_ file is the mentioned specially-crafted ZIP file as described above.
...
The exploit script above performs the TOCTOU attack to reset and receive the password of the **cyberx** username without authentication at all. It does so by utilizing three threads:
  ...
  * **looper_malicious** – the same as _looper_benign_ but uploads the malicious ZIP, in this configuration with a 1 second timeout
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image3-1.jpg)
...
From `components\xsense-web\cyberx_web\api\authentication.py:151`:
  ...
  # 2. validate new password=***REDACTED*** = body['new_password']
  ...
  # 3. change password=***REDACTED***
  user.save()
  **process.run('sudo /usr/local/bin/cyberx-users-password-reset -u {username} -p {password}'**
  .format(username=user.get_username().encode('utf-8'), password=***REDACTED*** hide_output=True)
...
This can be exploited with the following HTTP packet:
  ...
  Content-Length: 100
  Content-Type: multipart/form-data; boundary=***REDACTED-SUSPECT-TOKEN***  {"username" : "cyberx", "password" : "",
  "new_password": "``"}
...
As shown, the UUID parameter is not sanitized and formatted into an SQL query. There are a couple of classes which inherit `DynamicTokenAuthenticationBaseHandler` _._ The flow to the vulnerable functions actually exists in the token validation process.
...
It is worth noting that the function `execute_select_query` internally calls to the SQL `execute`, API which supports stacked queries. This makes the “simple” select SQL injection a more powerful primitive (_aka_ executing any query using `‘;’`). In our testing we managed to insert, update, and execute SQL special commands.
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/attack.jpg)
...
The sensor machine uses [RCDCAP](https://sourceforge.net/projects/rcdcap/) (an open source project) to open CISCO ERSPAN and HP ERM encapsulated packets.
...
![](https://www.sentinelone.com/wp-content/uploads/2022/03/image1-3.jpg)
...
## Impact
  ...
  * Who is affected? Azure Defender for IoT running with unpatched systems are affected. Since this product has many configurations, for example [RTOS](https://docs.microsoft.com/en-us/azure/defender-for-iot/device-builders/iot-security-azure-rtos), which have not been tested, users of these systems can be affected as well.
  * What is the risk? Successful attack may lead to _full network compromise_ , since Azure Defender For IoT is configured to have a TAP (Terminal Access Point) on the network traffic. Access to sensitive information on the network could open a number of sophisticated attacking scenarios that could be difficult or impossible to detect.

## Mitigation

We responsibly disclosed our findings to MSRC in June 2021, and Microsoft has released a security advisory with patch details December 2021, which can be found [here](https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-42310), [here](https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-42312), [here](https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-42313), [here ](https://msrc.microsoft.com/update-guide/vulnerability/CVE-2021-42311)and [here](http://hg.code.sf.net/p/rcdcap/code/rev/0af05606c55b#l2.10).
