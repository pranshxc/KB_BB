...
original_path: knowledge-inbox/blogs-incoming/2023-08-18_google-extensions-awarded-188337.md
original_filename: 2023-08-18_google-extensions-awarded-188337.md
...
- sso
language: en
raw_sha256: 88685cf4d4c47d5536456e094c3d6d8642145dbd3f6cf98022f19e17c808aa3b
text_sha256: 539125464185b4af4b1c648df882bd7d45edbf5b30ebf6773960c55cd1768401
...
- Original Path: knowledge-inbox/blogs-incoming/2023-08-18_google-extensions-awarded-188337.md
- Source Type: markdown
- Detected Topics: xss, csrf, mobile-security, supply-chain, oauth, sso
- Ingested At: 2026-06-28T07:32:25Z
- Redactions Applied: True
- Raw SHA256: `88685cf4d4c47d5536456e094c3d6d8642145dbd3f6cf98022f19e17c808aa3b`
- Text SHA256: `539125464185b4af4b1c648df882bd7d45edbf5b30ebf6773960c55cd1768401`
...
---
title: "Google Extensions (Awarded $18833.7)"
page_title: "Google Extensions (Awarded $18833.7) | Writeups"
url: "https://ndevtk.github.io/writeups/2023/08/18/extensions/"
final_url: "https://ndevtk.github.io/writeups/2023/08/18/extensions/"
...
My bad attempt at enforcing site isolation for SponsorBlock with its 3rd party player support [ajayyy/SponsorBlock#1784](https://github.com/ajayyy/SponsorBlock/pull/1784) Interesting article related to V8 isolates and process isolation [The Cloudflare Workers Security Model](https://blog.cloudflare.com/mitigating-spectre-and-other-security-threats-the-cloudflare-workers-security-model/)
...
Run the following on any google subdomain including insecure `http://` ones. The following code runs a VBS script on the victim resulting in RCE.
...
A demo to get RCE: [extension](https://github.com/NDevTK/DriveRCE) [video](https://www.youtube.com/watch?v=BI-szRhjO3g)  
...
Perfetto UI 1 is a Chrome extension by Google for recording browser traces. The extension communicates with ui.perfetto.dev to record the browser traces. This extension is particularly powerful as it is hardcoded to receive special treatment from Chrome (`ExtensionIsTrusted`). 2
...
Steps to reproduce:
  ...
  1. Install the Perfetto UI extension 1
...
### Attack scenario
  ...
  1. An attacker sends a link to the page to a victim who has the Perfetto UI 1 extension installed.
...
It exposes various commands that can be called. The issue is that the message listeners do not check the origin of the incoming message. 5 One of these commands is `clickNodeRef` 6, which can, with the help of selector injection 7, be used to click any element in the DOM, using an arbitrary selector.
...
### Attack scenario
  ...
  1. The attacker sends a link to a victim who uses the Screen Reader accessibility extension. 3
...
The new security check has a list of allowed origins however there’s open redirects! However this only worked for the `application/javascript` content type.
...
Steps to reproduce:
  ...
  1. Observe that the document tree gets leaked.

### Attack scenario

  1. An attacker sends a link to the page with this payload to a victim who has the Form Troubleshooter 9 extension installed.
...
Click the start button via the extensions popup, then go to `https://example.org/?secret` In the context of the content script on an attacker controlled website.
...
Steps to reproduce:
  ...
  2. Open <https://vuln-long-desc-extension.websec.blog/poc.html>
  ...
  1. Right click and select “Open Long Description In New Tab”
...
This is an extension by Google intended for frontend Gerrit developers. <https://gerrit.googlesource.com/gerrit-fe-dev-helper/>  
...
**URL:** <https://chrome.google.com/webstore/detail/chrome-reporting-extensio/emahakmocgideepebncgnmlmliepgpgb> This extension requires setup to work: Create DWORD `report_user_browsing_data` with the value of `1` in `HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Google\Chrome\3rdparty\extensions\emahakmocgideepebncgnmlmliepgpgb\Policy` In the context of the content script on an attacker controlled site run:
...
`chrome-extension://gmbmikajjgmnabiglmofipeabaddhgne/upload.html?aid=image-entire&tid=<TabID>`. By changing the action id `aid` from image-entire to html, it will leak the source code of the page to the folder. This is exploitable from a browser extension with the Tabs API.
...
Steps to reproduce:
  ...
  1. Make sure the Secure Shell extension 11 is installed
  2. Open poc.html
...
If the developer mode is enabled, the `shell` command runs and gives access to the system shell. In the regular user mode, the Crosh shell allows executing any of the predefined commands, which could possibly be exploited to gain access to the system shell.
...
To get a Linux shell in the Termina VM, run in crosh: `vmc start termina` To get a Linux shell in a container, run in crosh: `vmc container termina test-t1 --timeout 120` (Note that `vmc container` commands usually need higher timeouts on my physical Chromebook, YMMV)
...
This was fixed by adding `frame-ancestors 'self';` to the extensions `content_security_policy`. <https://chromium-review.googlesource.com/c/apps/libapps/+/4603751>
...
args from the url gets base64 decoded and set as attributes for a html embed tag. While JS is prevented due to the CSP it still allows for CSS. <https://issuetracker.google.com/issues/260531249>
...
Unverified fixes, [https://github.com/search?q=repo%3AProtonMail%2FWebClients+SECBTY-628&type=commits](https://github.com/search?q=repo%3AProtonMail%2FWebClients+SECBTY-628&type=commits)
...
The summary of the attack:
  ...
  3. Attacker invites the user (with the Google Optimize extension installed) to their account, which causes this code to execute after redirecting to any site. The UXSS runs when you click on the blue edit button. see picture below: <https://i.imgur.com/bGXJ5VF.png> The token for the payload was stored in chrome.storage.local so the user interaction (which got WAI) is not needed if you have a compromised renderer. <https://www.youtube.com/watch?v=h1zTOBpMjmw>
...
This may Include extensions that are not considered official google apps.
  ...
  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  ***REDACTED-SUSPECT-TOKEN***  1. <https://chrome.google.com/webstore/detail/perfetto-ui/lfmkphfpdbjijhpomgecfikhfohaoine> ↩ ↩2 ↩3
  ...
  3. <https://chrome.google.com/webstore/detail/screen-reader/kgejglhpjiefppelpmljglcjbhoiplfn> ↩ ↩2
  ...
  9. <https://chrome.google.com/webstore/detail/form-troubleshooter/lpjhcgjbicfdoijennopbjooigfipfjh> ↩ ↩2

  10. <https://github.com/GoogleChromeLabs/form-troubleshooter> ↩

  11. <https://chrome.google.com/webstore/detail/secure-shell/iodihamcpbpeioajjeobimgagajmlibd> ↩ ↩2

  12. <https://chrome.google.com/webstore/detail/deprecated-secure-shell-a/pnhechapfaindjhompbnflcldabbghjo> ↩

[Improve this page](https://github.com/NDevTK/writeups/edit/main/_posts/2023-08-18-extensions.md) Default Random Basic Shader Wallpapers Chrome Cast Surveillance Chromium Advisories Side Channel Echo Grove ParentalControlLock Light Cyberpunk Code Matrix Blueprint Redacted Glitch Old Terminal Gradient Comic Base64 Emoji Minecraft Flip 🦆 Rainbow text Typoifier Summarizer AI Audio Spoof Oceanic Depths Retro Gamification NoScript
